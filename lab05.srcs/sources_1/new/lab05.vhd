...
entity lab05 is
    port(
        ...
        --pmod joystick
        cs : buffer STD_LOGIC_VECTOR(0 downto 0);  --??? not sure what type chip select is
        miso : in STD_LOGIC;
        sclk : buffer STD_LOGIC;     --??? not sure what type sclk is 
        mosi : out STD_LOGIC
    );        
end lab05;
...
architecture Behavioral of lab05 is
    ...
    signal reset_n, x_position, y_position, trigger_button, center_button : std_logic;          
    signal jstck_direct : integer; --up = 2, stop = 1, down = 0       


begin
    -- Port map the pmodjoystick
    joystick : pmod_joystick generic map (clk_freq => 50) port map(clk, reset_n, miso, mosi, sclk, cs, x_position, y_position, trigger_button, center_button);
    ...
    -- TODO transfer y_position to direction
    -- process (y_position)
    -- begin
